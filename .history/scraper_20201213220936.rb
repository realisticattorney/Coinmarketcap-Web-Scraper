...
def scraper
   ...
   currencies_listing = parsed_page.css('tbody tr')
   page = 1
   ...
   while page <= last_page
      ...
      pagination_currencies_listing  = pagination_parsed_page.css('tbody tr')

      pagination_currencies_listing.each do |currency_index|
      currency_index = i
      currency = {
         price: parsed_page.css('tbody tr td div.price___3rj7O a.cmc-link')[currency_index].text,
         name: parsed_page.css('tbody tr td div.price___3rj7O a')[currency_index].attributes["href"].value
      }
      i += 1
      currencies << currency
      end
