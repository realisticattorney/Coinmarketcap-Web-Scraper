  ...
  def scraper
    ...
    while page <= last_page && page <= 1
      ...
      pagination_currencies_listing.count.times do |currency_index|
        ...
        when 'Derivatives'
         name = pagination_parsed_page.css('div.grid div.ds1a36-0.izflST ul li.market-pair-section h3.sc-1m8sms1-2.lcrPJw.market-pair-section-title a')[currency_index].attributes['href'].value
         # name = name.split('/')[-2].upcase
         currency = [name.to_s]
        when 'DeFi'
          currency = {
