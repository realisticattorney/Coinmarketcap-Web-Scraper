  ...
  def scraper
    ...
    while page <= last_page && page <= 3
      ...
      pagination_currencies_listing.count.times do |currency_index|
        case @data_type.to_s
        when 'Crypto'
            name = pagination_parsed_page.css('tbody tr td div.price___3rj7O a.cmc-link')[currency_index].text
            price = pagination_parsed_page.css('tbody tr td div.price___3rj7O a')[currency_index].attributes['href'].value
            currency = [name.to_s, name.to_s, price.to_s]
        when 'Derivatives'
          currency = {
