    ...
    while page <= last_page && page <= 1
      ...
      pagination_currencies_listing.count.times do |currency_index|
        ...
        when 'Derivatives'
         ...
         name = pagination_parsed_page.css('tbody')[currency_index].css('tr td div.price___3rj7O')[currency_index].text
         # name = pair.css('tr td div.price___3rj7O')[currency_index].text
         # name = name.split('/')[-2].upcase
         currency = [name.to_s, price.to_s]
        when 'DeFi'
          currency = {
