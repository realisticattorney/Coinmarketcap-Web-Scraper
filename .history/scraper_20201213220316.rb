...
def scraper
   ...
   while page <= last_page
      ...
      currencies_listing.each do |currency|
      currency = i
      currency = {
         price: prices = parsed_page.css('tbody tr td div.price___3rj7O a.cmc-link')[currency].text,
         name: parsed_page.css('tbody tr td div.price___3rj7O a')[currency].attributes["href"].value
      }
      i += 1
      currencies << currency
      end
